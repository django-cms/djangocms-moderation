...
class VersionLockingTestCase(BaseTestCase):
    def test_is_content_version_locked(self):
        version = PageVersionFactory(created_by=self.user)
        self.assertTrue(is_content_version_locked(version.content, self.user))
        self.assertFalse(is_content_version_locked(version.content, self.user2))
        version.publish(self.user)
        self.assertTrue(is_content_version_locked(version.content, self.user2))
        ...
        with mock.patch('djangocms_version_locking.helpers.content_is_unlocked_for_user') as _mock:
            is_content_version_locked(version.content, self.user2)
