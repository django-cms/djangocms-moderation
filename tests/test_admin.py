...
class ModerationAdminTestCase(BaseTestCase):
    ...
    def test_publish_selected_action_visibility_when_mr_is_published(self):
        mock_request = MockRequest()
        mock_request.user = self.user
        mock_request._collection = self.collection
        ...
        # Now, when version becomes published, they should not see it
        self.mr1.version._set_publish(self.user)
        self.mr1.version.save()

        actions = self.mra.get_actions(request=mock_request)
        # mr1 request is approved and published so user can't see
        # the publish_selected action
